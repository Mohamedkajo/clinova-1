...
export function renderAppointmentDetails({ language = "he", status = "ready", appointment = null, clinicalVisit = null, canOpenClinicalVisit = false, error = "", canChangeStatus = false } = {}) {
  ...
  if (status === "loading") {
    ...
    content = `<div class="appointment-detail-content">
      ...
      ${appointment.consentStatus ? `<section class="appointment-consent-status" aria-label="${escapeAttribute(language === "he" ? "סטטוס הסכמה" : language === "ar" ? "حالة الموافقة" : "Consent status")}"><strong>${escapeHtml(language === "he" ? "הסכמה לפני טיפול" : language === "ar" ? "الموافقة قبل العلاج" : "Consent before treatment")}</strong><span class="pill ${appointment.consentStatus.complete ? "done" : "pending"}">${escapeHtml(appointment.consentStatus.complete ? (language === "he" ? "הושלם" : language === "ar" ? "مكتمل" : "Complete") : (language === "he" ? "ממתין" : language === "ar" ? "قيد الانتظار" : "Pending"))}</span>${appointment.consentStatus.missing?.length ? `<small>${appointment.consentStatus.missing.map((item) => escapeHtml(item.title)).join(", ")}</small>` : ""}</section>` : ""}
